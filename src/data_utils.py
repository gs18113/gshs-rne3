...
def prepare_data(init_data, source_vocab, target_vocab, input_format, output_format, source_serialize, target_serialize, pointer_gen, n_cpus):
  ...
  for prog in init_data:
    ...
    ret = build_vocab_oovs(source_prog, copy.deepcopy(source_vocab), copy.deepcopy(target_vocab), input_format) if pointer_gen else (None, None)
    (vocab_oovs, target_vocab_extended) = ret
