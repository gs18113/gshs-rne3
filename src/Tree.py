...
class TreeManager(object):
    ...
    def build_binary_tree_from_dict_oovs(self, init_tree, parent=None, depth=0):
        ...
        if num_children == 0:
            ...
            self.trees[current_id].lchild = lchild_id
            return current_id
        first_child_id = self.build_binary_tree_from_dict_oovs(init_tree['children'][0], current_id, depth + 1)
        self.trees[current_id].lchild = first_child_id
        pre_child_id = first_child_id
        for i in range(1, len(init_tree['children'])):
            current_child_id = self.build_binary_tree_from_dict_oovs(init_tree['children'][i], pre_child_id, self.trees[pre_child_id].depth + 1)
            self.trees[pre_child_id].rchild = current_child_id
            pre_child_id = current_child_id
...
def trees_strip(data):
    ret = []
    for prog in data:
        prog['source_ast'] = json_strip(prog['source_ast'])
